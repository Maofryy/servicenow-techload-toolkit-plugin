...
For prose accents, see "Prose Page Elements" in `../../knowledge-commons/visual/references/css-patterns.md`. For everything else, use the standard freeform approach with aesthetic directions below.
...
**For pages with 4+ sections** (reviews, recaps, dashboards), also read `../../knowledge-commons/visual/references/responsive-nav.md` for section navigation with sticky sidebar TOC on desktop and horizontal scrollable bar on mobile.
...
**Mermaid theming:** Always use `theme: 'base'` with custom `themeVariables` so colors match your page palette. Use `layout: 'elk'` for complex graphs (requires the `@mermaid-js/layout-elk` package — see `../../knowledge-commons/visual/references/libraries.md` for the CDN import). Override Mermaid's SVG classes with CSS for pixel-perfect control. See `../../knowledge-commons/visual/references/libraries.md` for full theming guide.
...
**Mermaid layout direction:** Prefer `flowchart TD` (top-down) over `flowchart LR` (left-to-right) for complex diagrams. LR spreads horizontally and makes labels unreadable when there are many nodes. Use LR only for simple 3-4 node linear flows. See `../../knowledge-commons/visual/references/libraries.md` "Layout Direction: TD vs LR".
...
See `../../knowledge-commons/visual/references/css-patterns.md` for image container styles (hero banners, inline illustrations, captions).
...
**Typography is the diagram.** Pick a distinctive font pairing from the list in `../../knowledge-commons/visual/references/libraries.md`. Every page should use a different pairing from recent generations.
...
**Text-heavy overviews (under 15 elements):** CSS Grid with explicit row/column placement. Sections as rounded cards with colored borders and monospace labels. Vertical flow arrows between sections. The reference template at `../../knowledge-commons/visual/templates/architecture.html` demonstrates this pattern. Use when cards need descriptions, code references, tool lists, or other rich content that Mermaid nodes can't hold.
...
Use a real `<table>` element — not CSS Grid pretending to be a table. Tables get accessibility, copy-paste behavior, and column alignment for free. The reference template at `../../knowledge-commons/visual/templates/data-table.html` demonstrates all patterns below.
...
Card grid layout. Hero numbers large and prominent. Sparklines via inline SVG `<polyline>`. Progress bars via CSS `linear-gradient` on a div. For real charts (bar, line, pie), use **Chart.js via CDN** (see `../../knowledge-commons/visual/references/libraries.md`). KPI cards with trend indicators (up/down arrows, percentage deltas).
...
**Before generating slides**, read `../../knowledge-commons/visual/references/slide-patterns.md` (engine CSS, slide types, transitions, nav chrome, presets) and `../../knowledge-commons/visual/templates/slide-deck.html` (reference template showing all 10 types). Also read `../../knowledge-commons/visual/references/css-patterns.md` for shared patterns and `../../knowledge-commons/visual/references/libraries.md` for Mermaid/Chart.js theming.
...
```bash
bash <skill-base-dir> (use the path shown in "Base directory for this skill:" above)/scripts/share.sh <html-file>
```

**Example:**

```bash
bash <skill-base-dir> (use the path shown in "Base directory for this skill:" above)/scripts/share.sh ~/.agent/diagrams/my-diagram.html
...
Before delivering, read `../../knowledge-commons/visual/references/anti-patterns.md` using the Read tool and verify the output passes the Slop Test. The file covers forbidden typography, color effects, layout patterns, and template clichés that signal "AI-generated" output — reviewing it before delivery is mandatory, not optional.
